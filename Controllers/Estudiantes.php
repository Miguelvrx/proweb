...
class Estudiantes extends Controller
{
    ...
    public function registrar()
    {
        ...
        if (empty($codigo) || empty($dni) || empty($nombre) || empty($carrera)) {
            ...
            if ($id == "") {
                    ...
                    if ($data == "ok") {
                        $msg = array('msg' => '¡Estudiante registrado exitosamente!', 'icono' => 'success');
                    } else if ($data == "existe") {
                        $msg = array('msg' => 'Este estudiante ya existe.!', 'icono' => 'warning');
                    } else {
                        $msg = array('msg' => 'Error al registrar estudiante!', 'icono' => 'error');
                    }
            } else {
                $data = $this->model->actualizarEstudiante($codigo, $dni, $nombre, $carrera, $direccion, $telefono, $id);
                if ($data == "modificado") {
                    $msg = array('msg' => '¡El estudiante cambió exitosamente!', 'icono' => 'success');
                } else {
                    $msg = array('msg' => '¡Error al cambiar de estudiante!', 'icono' => 'error');
                }
            }
        }
    ...
    public function eliminar($id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => '¡Estudiante dado de alta con éxito!', 'icono' => 'success');
        } else {
            $msg = array('msg' => '¡Error al despedir al alumno!', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    }
    public function reingresar($id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => '¡El estudiante reingresó exitosamente!', 'icono' => 'success');
        } else {
            $msg = array('msg' => '¡Error al reingresar estudiante!', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    }

...
class Materia extends Controller
{
    ...
    public function registrar()
    {
        ...
        if (empty($materia)) {
            $msg = array('msg' => '¡El campo de nombre es obligatorio!', 'icono' => 'warning');
        } else {
            if ($id == "") {
                $data = $this->model->insertarMateria($materia);
                if ($data == "ok") {
                    $msg = array('msg' => '¡Artículo registrado exitosamente!', 'icono' => 'success');
                } else if ($data == "existe") {
                    $msg = array('msg' => 'Este artículo ya existe.!', 'icono' => 'warning');
                } else {
                    $msg = array('msg' => 'Error al registrar la materia!', 'icono' => 'error');
                }
            } else {
                $data = $this->model->actualizarMateria($materia, $id);
                if ($data == "modificado") {
                    $msg = array('msg' => '¡Artículo cambiado exitosamente!', 'icono' => 'success');
                } else {
                    $msg = array('msg' => 'Error al modificar la materia!', 'icono' => 'error');
                }
            }
        }
    ...
    public function eliminar($id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => 'Materia dada de baja!', 'icono' => 'success');
        } else {
            $msg = array('msg' => 'Error al dar de baja la materia!', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    }
    public function reingresar($id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => 'Materia Reingresada correctamente!', 'icono' => 'success');
        } else {
            $msg = array('msg' => 'Error al reingresar la materia!', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    }

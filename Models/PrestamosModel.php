...

{
    ...
    {
        $sql = "SELECT e.id, e.nombre, l.id, l.titulo, p.id, p.id_estudiante, p.id_libro, DATE_FORMAT(p.fecha_prestamo,'%d/%m/%Y') AS data_emprestimo, DATE_FORMAT(p.fecha_devolucion,'%d/%m/%Y') AS data_devolucao, p.cantidad, p.observacion, p.estado FROM estudiante e INNER JOIN libro l INNER JOIN prestamo p ON p.id_estudiante = e.id WHERE p.id_libro = l.id";
    ...
    {
        $sql = "SELECT e.id, e.nombre, l.id, l.titulo, p.id, p.id_estudiante, p.id_libro, DATE_FORMAT(p.fecha_prestamo,'%d/%m/%Y') as data_emprestimo, DATE_FORMAT(p.fecha_devolucion,'%d/%m/%Y') AS data_devolucao, p.cantidad, p.observacion, p.estado FROM estudiante e INNER JOIN libro l INNER JOIN prestamo p ON p.id_estudiante = e.id WHERE p.id_libro = l.id AND p.estado = 1 ORDER BY e.nombre ASC";
        $res = $this->selectAll($sql);
        return $res;
    }
    ...
    {
        $sql = "SELECT e.id, e.codigo, e.nombre, e.carrera, l.id, l.titulo, p.id, p.id_estudiante, p.id_libro,  DATE_FORMAT(p.fecha_prestamo,'%d/%m/%Y') as data_emprestimo, p.fecha_devolucion, p.cantidad, p.observacion, p.estado FROM estudiante e INNER JOIN libro l INNER JOIN prestamo p ON p.id_estudiante = e.id WHERE p.id_libro = l.id AND p.id = $id_prestamo";
        $res = $this->select($sql);
        return $res;
    }
}
